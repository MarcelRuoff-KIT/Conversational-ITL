...
export class UnitedStatesMapComponent implements OnInit, AfterViewInit {
  ...
  statesSelect = ["Alabama", "Alaska", "Arizona", "Arkansas", "California", "Colorado", "Connecticut", "Delaware", "Florida", "Georgia", "Hawaii", "Idaho", "Illinois", "Indiana", "Iowa", "Kansas", "Kentucky", "Louisiana", "Maine", "Maryland", "Massachusetts", "Michigan", "Minnesota", "Mississippi", "Missouri", "Montana", "Nebraska", "Nevada", "New Hampshire", "New Jersey", "New Mexico", "New York", "North Carolina", "North Dakota", "Ohio", "Oklahoma", "Oregon", "Pennsylvania", "Puerto Rico", "Rhode Island", "South Carolina", "South Dakota", "Tennessee", "Texas", "Utah", "Vermont", "Virginia", "Washington", "West Virginia", "Wisconsin", "Wyoming"];
  datesSelect = ["2021-06-30"];
  ...
  public createSVG() {
    ...
    this.svg = d3
      ...
      .attr("style", 'display: block; margin: auto; position: absolute; left: 10px; top: 120px;')
      .on("click", this.stopped, true);
  }

  async updateMap(mode) {
    document.getElementById("requiredVisPar")["style"]["display"] = "none"
    document.getElementById("loading")["style"]["display"] = "block"
    this.childUpdateMap(mode)
  }

  async childUpdateMap(mode) {

    this.width = document.getElementById("map").clientWidth - 100
    this.height = document.getElementById("map").clientHeight - 75//125

    this.legendContainerSettings.y = this.height
    ...
    if (this.legend_Values != "State" && this.legend_Values != "Date") {
      updateNecessary = false;
      that.newCases = []
    }

    if (this.y_Axis_Values.length == 0) {
      updateNecessary = false;
      that.newCases = []
    ...
    try {
      if (this.chartType == "barChart") {
        ...
        if (((JSON.stringify(this.filterBar) != JSON.stringify(filterBar)) && mode == "update") || mode == "normal") {
          ...
          for (var i = 0, n = dataColumns.length; i < n; i++) {
            ...
            state_name.selectAll(".bar." + dataColumns[i])
              ...
              .append("rect")
              .attr("class", d => {
                return "bar " + dataColumns[i] + "|" + d[groupByLegend]
              })
              .attr("fill", z(dataColumns[i]))
              .attr("x", d => xScale1(dataColumns[i]) + this.unqiueGroupArray.indexOf(d[groupByLegend]) * width)
      ...
      /**/
        ...
        if (((JSON.stringify(this.filterBar) != JSON.stringify(filterScatter)) && mode == "update") || mode == "normal") {
          this.filterBar = filterScatter
          ...
          const x = d3.scaleLinear()
            ...
            var max_Value = 1.1 * d3.max(that.newCases, d => d[y_Axis])


          this.svg.append("g")
            .attr("transform", "translate(0," + this.height + ")")
            .call(d3.axisBottom(x).ticks(7).tickFormat(function (d: number) {
              if (max_Value > 700000) {
                var s = formatSimpleNumber(d / 1e6);
                return this.parentNode.nextSibling
                  ? "\xa0" + s
          ...
          const y = d3.scaleLinear()
            ...
            max_Value = 1.1 * d3.max(that.newCases, d => d[y_Axis])


            var yAxis = d3.axisLeft(y).ticks(10).tickSizeOuter(0)
            .tickFormat(function (d: number) {
              if (max_Value > 700000) {
                var s = formatSimpleNumber(d / 1e6);
                return this.parentNode.nextSibling
          ...
          dots.selectAll("dot")
            ...
            .attr("cy", d => { if (typeof d[y_Axis] != 'undefined') { return y(d[y_Axis]) } else { return 250 } })
            .attr("r", 7)
            .style("opacity", .7)
            .style("fill", function (d) {
              if (legend == "State") {
                ...
                return d3.interpolatePuRd(((new Date(d[legend]).getTime() - new Date("2020-01-01").getTime()) / (new Date("2021-06-30").getTime() - new Date("2020-01-01").getTime())))
              }
            })
          ...
          legend_Axis.append("rect")
            ...
            .attr("fill", function (d) {
              if (legend == "State") {
                ...
                return d3.interpolatePuRd(((new Date(d).getTime() - new Date("2020-01-01").getTime()) / (new Date("2021-06-30").getTime() - new Date("2020-01-01").getTime())))
              }
            });
        ...
        if (((JSON.stringify(this.filterBar) != JSON.stringify(filterMap)) && mode == "update") || mode == "normal") {
          this.filterBar = filterMap
          ...
          that.g
            ...
            .on("mouseover", function (d) {
              ...
              that.tooltip
                .html(
                  d.name + "<br/>" + that.datesSelect[0] + "<br/><b>Total " + y_Axis + ":</b> " + that.formatDecimal(d.metric)
                )
                .style("left", d3.event.pageX + "px")
                .style("top", d3.event.pageY + "px");
          ...
          legend_Axis
            ...
            .attr("y", that.legendContainerSettings.y + 25)
      ...
      console.log(e)
      console.log("Failed Data Loading")
    }
  ...
  closeFollowUp() {
    $('#followUpNotification').css("display", "none")
  }
  ...
  highlightMessage() {
    ...
    try {
      ...
      if (text.startsWith("Bot said:Did you mean") || text.startsWith("Bot said:If there were") || text.startsWith("Bot said:Do you also")) {
